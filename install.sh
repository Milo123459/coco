...
get_tmpfile() {
  ...
  suffix="$1"
  if has mktemp; then
    printf "%s%s.%s.%s" "$(mktemp)" "-coco" "${RANDOM}" "${suffix}"
  else
    # No really good options here--let's pick a default + hope
    printf "/tmp/coco.%s" "${suffix}"
  fi
}
...
download() {
  ...
  printf "\n" >&2
  info "This is likely due to Coco not yet supporting your configuration."
  info "If you would like to see a build for your configuration,"
  info "please create an issue requesting a build for ${MAGENTA}${TARGET}${NO_COLOR}:"
  info "${BOLD}${UNDERLINE}https://github.com/Milo123459/coco/issues/new/${NO_COLOR}"
  return $rc
}
...
unpack() {
  ...
  info "bug report at https://github.com/Milo123459/coco/issues"
  return 1
}
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing Coco, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing Coco as root, please wait…"
  fi
  info "$msg"
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for Coco"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/Milo123459/coco/issues/new/${NO_COLOR}"
...
UNINSTALL=0
HELP=0
CARGOTOML="$(curl -fsSL https://raw.githubusercontent.com/Milo123459/coco/master/Cargo.toml)"
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/Milo123459/coco/releases"
fi
...
if [ $UNINSTALL == 1 ]; then
  confirm "Are you sure you want to uninstall coco?"
  ...
  if test_writeable "$(dirname "$(which coco)")"; then
    sudo=""
    msg="Removing Coco, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Removing Coco as root, please wait…"
  fi
...
URL="${BASE_URL}/latest/download/coco-${TARGET}.${EXT}"
info "Tarball URL: ${UNDERLINE}${BLUE}${URL}${NO_COLOR}"
confirm "Install Coco ${GREEN}${VERSION[0]}${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install "${EXT}"
completed "Coco installed"